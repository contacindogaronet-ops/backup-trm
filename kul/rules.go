...
func FetchRulesFromGithub() {
	...
	for path, tier := range files {
		...
		if err != nil {
			if LogErrors {
				log.Warn().Str("file", path).Msg("File rules tidak ditemukan")
			}
			continue
		}
...
func injectToDB(line []byte, tier byte) {
	...
	// Buang baris komentar (#) dan exception (!)
	...
	// Hapus spasi dan ^ (EasyList/Adguard modifier) di ujung
	...
	// Hapus '||' di awalan (EasyList format)
	if len(line) >= 2 && line[0] == '|' && line[1] == '|' {
		line = line[2:]
	}

	// Tangani format "/etc/hosts" (misal: "0.0.0.0 ads.com")
	// Ambil kata paling belakang saja sebagai domain
	lastSpace := -1
	...
	// Cek apakah ini IP/CIDR atau murni Domain
	isIP := true
	hasSlash := false
	for i := 0; i < len(line); i++ {
		b := line[i]
	...
	// Jika murni domain, simpan bersih ke Fastcache
