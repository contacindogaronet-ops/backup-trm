...
class SmartDownloadManager {
    ...
    // Dedicated Worker dengan Auto-DC Migration (Kebal FileMigrateError)
    async executeWorker(workerId, fileLocation, startPart, endPart, chunkSize, partsBuffer) {
        ...
        for (let i = startPart; i < endPart; i++) {
            ...
            while (!success && retries > 0) {
                try {
                    let result;
                    // Jika worker mendeteksi perpindahan DC, gunakan sender khusus DC tersebut
                    if (currentSender) {
                    ...
                    // Deteksi jika file berada di Data Center berbeda (FileMigrateError)
